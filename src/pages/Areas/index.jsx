...
export default function AreasPage() {
  ...
  useEffect(() => { setPage(1); }, [search, floorFilter]);

  // Backend already filters; derive grouped/paged from the returned array
    ...
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-4" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.04)" }}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {floors.map((floor) => {
            ...
            return (
              <button key={floor.id} onClick={() => setFloorFilter(active ? "all" : floor.id)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold border transition-all"
                style={active
            ...
            <div key={floor}>
              ...
              {view === "grid" ? (
                  ...
                  <AnimatePresence mode="popLayout">
                    {items.map((area, i) => (
                      <motion.div key={area.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <AreaCard area={area} onEdit={() => setModal(area)} onDelete={() => setDelTarget(area)} />
                      </motion.div>
                    ))}
                  </AnimatePresence>
                </div>
                ...
                <div className="mt-3 bg-white rounded-2xl border border-slate-100 overflow-hidden" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
                  <ListHeader />
                  {items.map((area, i) => (
                    <AreaRow key={area.id} area={area} last={i === items.length - 1}
                      onEdit={() => setModal(area)} onDelete={() => setDelTarget(area)} />
                  ))}
                </div>
              )}
            </div>
      ...
      {areas.length > 0 && !showGrouped && view === "list" && (
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
          <ListHeader />
          {pagedItems.map((area, i) => (
            <AreaRow key={area.id} area={area} last={i === pagedItems.length - 1}
              onEdit={() => setModal(area)} onDelete={() => setDelTarget(area)} />
          ))}
        </div>
      )}
...
/* ── Floor Select (custom dropdown with inline Add Floor) ───── */
// value = floorId string; onChange(id, name) sets both
...
/* ── Floor Add Modal (mirrors Floors page modal) ─────────────── */
function FloorModal({ open, floor, onClose, onSave, floors, propertyName }) {
  const { register, handleSubmit, reset, watch, setValue } = useForm();
  const selectedColor = watch('color', '#0b1d3a');
        ...
        <Field label="Accent Color">
          ...
          <div className="mt-3 flex items-center gap-3 px-4 py-3 rounded-xl"
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">{levelLabel(parseInt(watch('level') ?? 0, 10))}</p>
            </div>
          </div>
          <input type="hidden" {...register('color')} />
        </Field>
        <FormActions onCancel={onClose} submitLabel="Add Floor" />
      </form>
    </Modal>
  );
}
...
function AreaRow({ area, last, onEdit, onDelete }) {
  const meta       = typeMeta(area.type);
  const assetCount = area.assetCount ?? 0;
  return (
    <Link to={`/areas/${area.id}`}
      className={cn("flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors group", !last && "border-b border-slate-50")}>
...
function AreaModal({ open, onClose, area, floors, propertyName, onSave, onAddFloor, pendingFloor, onClearPending }) {
  const { register, handleSubmit, reset, watch, setValue } = useForm();
  ...
  // Auto-select a newly created floor
  useEffect(() => {
  ...
  return (
    <Modal open={open} onClose={onClose} title={area ? "Edit Area" : "Add New Area"} subtitle={`Define a room or space in ${propertyName ?? 'your property'}`}>
      <form onSubmit={handleSubmit(handleSave)} className="space-y-5">
        <Field label="Area Name" required>
          <Input {...register("name", { required: "Required" })} placeholder="e.g. Master Bedroom" />
        </Field>
        <FormGrid>
          <Field label="Area Type" required>
            <Select {...register("type", { required: "Required" })} placeholder="Select type"
        ...
        {selectedType === 'Other' && (
          <Field label="Custom Type Name" required>
            <Input {...register("customType", { required: 'Required' })}
        ...
        <FormActions onCancel={onClose} submitLabel={area ? "Update Area" : "Add Area"} />
      </form>
    </Modal>
  );
}
